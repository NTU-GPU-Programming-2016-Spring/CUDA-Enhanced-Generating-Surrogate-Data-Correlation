...
for role_idx=1:1
	...
	file_name = sprintf('%s-pos.csv', roles{role_idx});
	file_dest = sprintf('./%s/%s', folder_name, file_name);
	brain_hist = csvread(file_dest);

	for brain_idx=1:20484
		file_dest = sprintf('./%s/%s', folder_name, file_name);
		dest = brain_hist(brain_idx,:);
		surr_size = size(dest, 2);
		p_val = length(find(dest >= m_cor(brain_idx)))/surr_size;
		md_p_val = length(find(dest >= md_cor(brain_idx)))/surr_size;
	...
	lh_inv_p_val_series = 1 - lh_p_val_series;
	rh_inv_p_val_series = 1 - rh_p_val_series;
